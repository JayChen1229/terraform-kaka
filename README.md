...
* **Linux**: 
  ...
  ```
* **Windows**: 下載 [Release EXE](https://github.com/getsops/sops/releases) 並加到 환경變數 PATH。
  
### 4. 離線環境 (Air-Gap) Provider 設定
為了配合封閉網路（內網），本專案的 `Jenkinsfile` 已設定為 **離線安裝 Provider**。
因此在第一次部署前，請務必在你**有網路**的開發機上執行以下打包指令，並將產生的資料夾一起 Commit 到 Git 裡：
```bash
# 下載 Linux_amd64 架構的 providers 到本地端
terraform providers mirror -platform=linux_amd64 terraform-provider-plugins/
git add terraform-provider-plugins/
```
